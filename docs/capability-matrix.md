...
| SBPA live workflow (lobby) | SAP Build lobby, environment "SCALE 2026" | Process "TrustSphere Human Review" (API trigger `StartReviewAPI`, inputs case_id/draft_id/evidence_summary, approval form, recipient user) built, released, deployed; instance started manually with CASE-19263 payload; approval task delivered to My Inbox (2026-07-30) | ✅ **LIVE** — human approval task executed in the tenant | Demonstrated live, human-triggered. Cockpit workflow record keeps the honest fallback label until the API trigger path (below) is unlocked. |
| SBPA API trigger (service key) | BTP cockpit, shared subaccount | (a) Create service instance / service key — refused (no service-management rights). (b) Environment API key created; live trigger POST to `spa-api-gateway-bpi-ap-prod.cfapps.ap11...workflow-instances?environmentId=scale2026` tested with `api-key`, `apikey`, `x-api-key`, and `Bearer <key>` headers — **all 401** (2026-07-30) | ❌ **BLOCKED & path exhausted** — OAuth bearer from an SBPA service key is mandatory; the environment API key is supplementary only | **Hybrid stance:** process live in the lobby (manual start + My Inbox approval, proven above); backend API-trigger code implemented + unit-tested with the real trigger URL/definitionId pre-configured — activating needs only `SBPA_SERVICE_KEY` once a subaccount admin issues one. Cockpit keeps the honest local-fallback label. |
