...
| SBPA live workflow (lobby) | SAP Build lobby, environment "SCALE 2026" | Process "TrustSphere Human Review" (API trigger `StartReviewAPI`, inputs case_id/draft_id/evidence_summary, approval form, recipient user) built, released, deployed; instance started manually with CASE-19263 payload; approval task delivered to My Inbox (2026-07-30) | ⚠️ **BUILT & EXECUTED, THEN RULED OUT** — organisers clarified (2026-07-31) that SAP Build is not available for team use in this shared tenant | **Demo uses the documented local review-state machine fallback** (CLAUDE.md §18), honestly labelled in the cockpit. The build/run above is retained as evidence the integration pattern works; do not demo or claim live SBPA. |
| SBPA API trigger (service key) | BTP cockpit, shared subaccount | (a) Create service instance / service key — refused (no service-management rights). (b) Environment API key created; live trigger POST to `spa-api-gateway-bpi-ap-prod.cfapps.ap11...workflow-instances?environmentId=scale2026` tested with `api-key`, `apikey`, `x-api-key`, and `Bearer <key>` headers — **all 401** (2026-07-30) | ❌ **CLOSED** — service key permission-blocked, and organisers subsequently ruled SAP Build out of team scope entirely (2026-07-31) | Backend trigger/sync code (`workflows/sbpa.py`) stays in the repo as target-state integration, unit-tested, dormant. **The workflow demo path is the local review-state machine fallback, labelled as such.** |
